...
const SCHOOL_RING_DEFS = [
  ...
  { id: 'ps_soc_3', name: '大学同学圈', cohort: 3, requiresUniversity: true }
];

function playerAttendedUniversity(g) {
  g = g || game;
  const edu = g && g.playerEducation;
  if (!edu) return true;
  return ['大专', '本科', '硕士', '博士'].indexOf(edu) >= 0;
}

function playerSchoolRingDefs(g) {
  g = g || game;
  const maxCohort = playerAttendedUniversity(g) ? 3 : 2;
  return SCHOOL_RING_DEFS.filter(function (r) {
    if (r.requiresUniversity && !playerAttendedUniversity(g)) return false;
    return r.cohort <= maxCohort;
  });
}

function classmateBirthYear(rng) {
  const by = (game && game.birthYear) || 1988;
  return by + Math.floor(rng() * 3) - 1;
}

function syncClassmateAgeToPlayer(c) {
  if (!c || !game || c.id === 'player') return;
  const by = game.birthYear || 1988;
  if (c.birthYear == null || Math.abs(c.birthYear - by) > 1) {
    const seed = ((game.stockSeed || 1) + String(c.id || '').length * 17) | 0;
    const rng = typeof dreamRng === 'function' ? dreamRng(seed) : Math.random;
    c.birthYear = classmateBirthYear(rng);
  }
}

function prunePlayerSchoolRingsBeyondEducation() {
  if (!game || !game.playerCircles) return;
  const allowed = {};
  playerSchoolRingDefs(game).forEach(function (r) { allowed[r.id] = true; });
  game.playerCircles.social = (game.playerCircles.social || []).filter(function (c) {
    if (!c || !c.id) return false;
    if (String(c.id).indexOf('ps_soc_') === 0) return !!allowed[c.id];
    return true;
  });
}
...
function generateClassmateNpc(ringId, index, cohort) {
  ...
  const birthYear = classmateBirthYear(rng);
  const contact = {
...
function populatePlayerSchoolCircles(force) {
  ...
  prunePlayerSchoolRingsBeyondEducation();
  const rings = playerSchoolRingDefs(game);
  let carry = [];
  rings.forEach(function (ring, ri) {
    ...
    members.forEach(function (m) {
      if (m.id && m.id !== 'player') {
        const c = circleMemberContact(m.id);
        if (c) syncClassmateAgeToPlayer(c);
      }
    });
...
function migrateSocialCircles() {
  ...
  (game.contacts || []).forEach(function (c) {
    if (c.kind === 'classmate') syncClassmateAgeToPlayer(c);
