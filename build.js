...
const GAME_BUILD_ID = '2026.06.09-stockfree';
  ...
  <section class="center">
    ...
    <div id="tabStock" style="display:none">
      <div class="finance-panel">
        <p style="color:var(--muted);margin-bottom:8px">A股真实标的（参考市价），佣金万分之2。股价围绕参考价波动。<b style="color:var(--green)">炒股随时可买卖，不占本周求职/赌博行动</b>，三个页签可随时切换。</p>
...
function serializeUiState(){
  return{actionDone,selectedIdx,selectedJobIdxs:[...selectedJobIdxs],applyCategoryPicks:[...applyCategoryPicks],
    currentSort,currentCategory,currentTab};
}
...
function createNewGame(slot,playerName,edu,school){
  ...
  game={
    ...
    casinoActive:false,marketActive:false,marketTimeLeft:0,marketApplyCount:0,marketBooths:[],
    ...
    wolfAchievement:false,expiredOfferCount:0,referralOpportunity:null,appSubscriptions:{},usedHeadhunterBatch:false
  };
...
function generateReferralOpportunity(){
  ...
  for(let n=0;n<count;n++){
    ...
    const offer={company:co,tier:co.tier,importance:op.importance,annualPay:op.pay,roleExtra:op.roleExtra,
      ...
      interviewCost:INTERVIEW_COST[op.importance]};
...
function renderReferralRow(item,checked){
  ...
  return '<label class="company-row picked"><input type="radio" name="referralPick" class="referral-pick" data-uid="'+item.uid+'"'+(checked?' checked':'')+'>'+
    ...
    '招：'+item.jobTitle+' · '+pay+' · 面试¥'+(o.interviewCost||0)+'<br>'+
...
function confirmReferralApply(){
  ...
  const replyIn=replyDelayWeeks(item.offer.tier,item.offer.importance);
  ...
  addLog('🤝 通过 '+item.referrer+'（'+item.relation+'）内推投递【'+item.jobTitle+'】@'+item.offer.company.name,'info');
  game.referralOpportunity=null;
  closeReferralModal();
  actionDone=true;
  finishWeek();
...
function generateRandomMarketBooth(){
  ...
  const offer={company:co,tier:co.tier,importance:op.importance,annualPay:op.pay,roleExtra:op.roleExtra,
    ...
    newToIndustry:!game.industryExperience[job.category],
    eduGap:Math.max(0,(EDU_RANK[job.education]||4)-(EDU_RANK[game.playerEducation]||4)),
    interviewCost:INTERVIEW_COST[op.importance],method:'market'};
...
function renderMarketBooth(item){
  ...
  return '<div class="market-booth"><div class="booth-body"><b>'+o.company.name+'</b> '+fmtCompanyBadge(o.company)+' · <b>'+imp+'</b><br>'+
    '招：<b>'+item.jobTitle+'</b>（'+item.category+'） · '+pay+' · 面试¥'+(o.interviewCost||0)+'<br>'+
...
function renderOffers(){
  ...
  list.innerHTML=game.offers.map(o=>{
    ...
    return '<div class="offer-item"><b>'+o.offer.company.name+'</b> · '+imp+' · '+formatOfferPay(o.offer)+
...
function updateOfferPreview(){
  ...
  if(hint){
    ...
    hint.textContent=txt+' · 面试：普通¥0/专家¥100/总监¥200 · 内推为独立按钮，偶发出现';
  }
  updateAppSubscriptionLabels();
...
function updateButtons(){
  ...
  if(hint){
    ...
    else if(!hasPick)hint.textContent='↑ 线上/猎头：先点击职业方块多选，或左侧点行业标签';
